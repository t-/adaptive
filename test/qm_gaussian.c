...
void write_gaussian_input(int step ,t_forcerec *fr, t_QMrec *qm, t_MMrec *mm)
{
  ...
  if(qm->bTS){
    fprintf(out,"OPT=(Redundant,TS,noeigentest,ModRedundant) Punch=(Derivatives) ");
  }
  else if (qm->bOPT){
    fprintf(out,"OPT=(Redundant,ModRedundant) Punch=(Derivatives) ");
  }
  else{
    fprintf(out,"FORCE Punch=(Derivatives) Pop=(MK,ESPDipole)");
  }
			  ...
			  t_QMrec *qm, t_MMrec *mm)
{
  ...
  FILE
    *in;
  system("sh make_fort8.sh"); 
  in=fopen("fort.8","r");
  fprintf(stderr,"reading fort.8 file!\n");
...
#else
  fprintf(stderr,"hijacted gromacs gaussian call\n");
  /*if ( system(buf) != 0 )*/
  /*gmx_fatal(FARGS,"Call to '%s' failed\n",buf);*/
 sprintf(out,"echo step %d >> clean.out",step);
 system(out);
 /*system("awk /Charge\\=\\ \\ -1.00000/,/\\-\\-\\-\\-\\-\\-/ input.log >>clean.out");*/
 /*system("awk /C10H9O2S1/,/PG\\=/ input.log >>clean.out");*/
#endif
}
